...
openssl11 req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /tmp/private.key -out /tmp/certificate.crt -subj /C=CA/ST=Ontario/L=Toronto/O=Cardioware/OU=Imaging/CN=$ComName/emailAddress=hmesha@gmail.com -addext extendedKeyUsage=serverAuth -addext subjectAltName=DNS:orthweb.cardioware.com,DNS:$ComName

runuser -l ec2-user -c "
  cd /home/ec2-user/ && git init orthweb && cd orthweb && \
  git remote add origin https://github.com/hmesha/orthweb.git && \
...
if [[ -f "/home/ec2-user/.s3_integration" ]]; then
  echo DOCKER_IMAGE=osimis/orthanc:22.2.2 >> .env
