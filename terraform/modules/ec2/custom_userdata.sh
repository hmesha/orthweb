...
openssl11 req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /tmp/private.key -out /tmp/certificate.crt -subj /C=CA/ST=Ontario/L=Waterloo/O=hmesha/OU=Imaging/CN=$ComName/emailAddress=info@hmesha.com -addext extendedKeyUsage=serverAuth -addext subjectAltName=DNS:orthweb.hmesha.com,DNS:$ComName
...
if [[ -f "/home/ec2-user/.s3_integration" ]]; then
  echo DOCKER_IMAGE=hmesha/orthanc >> .env
